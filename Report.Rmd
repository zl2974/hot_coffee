...
help reflect some of relationship in the data. To simplify our analysis but reserve the most information, we choose to limit our data to Vehicle types to _PAS_ as passenger's vehicle, and month to _July_.

```{r cluster_data, cache=T}
set.seed(1)
clus_df =
  parking %>%
  filter(lubridate::month(issue_date) == 7, hour <= 24) %>%
  mutate(weekday = weekdays(issue_date)) %>% 
  select(
    hour,
    weekday,
    registration_state,
...
factoextra::fviz_nbclust(clus_df, FUNcluster = cluster::pam, method = "wss")

factoextra::eclust(clus_df, "pam", k = 9, k.max = 10)
...
\ Categorical data is in this datasets, so Gower Distance is used to adapt 
our data structure.
```{r matrix ,cache=T}
