...
Our question might shifted a bit during the exploration and regression, due to either falling into a rabbit hole or the data limitation( or our limitation). Although these questions could be quiet common in different cities, here we will focus on NYC since it's a typical one.

## Data

### Data Sorce

As we mentioned before, our project will focus on data in NYC, and all data was downloaded from [NYC Open Data](https://opendata.cityofnewyork.us/).  
...
This dataset contains 380 million violations issued during the 2021 fiscal year.Data is provided by the Department of Finance (DOF).Each record represents a single violation issued. Information on summond number, issue date, violation code and issue street, house number,etc. are recorded. The Issuance datasets are not updated to reflect violation status, the information only represents the violation(s) at the time they are issued.

A ParkingViolation Code excel is attached to this dataset, which records corresponding violation information, such as description and fine amount, of each violation code.
...
\ First, in order to display these violations in a visualized way, we focus on acquiring our own geographic information via available information in the datasets. 
...
Manhattan 96th St. & below, we select them by 96th St. geographic variables, and acquire fine amount according to the attached Parking Violation Code excel.
