...
vio = parking %>%
  ggplot(aes(x = lat, y = long,color = borough),alpha = 0.0001)+
  geom_jitter() +
  xlab("altitude") +
  ylab("longitutde")
...
parking %>% 
  ...
  plot_ly(x = ~x, y = ~y, z = ~z) %>% 
    add_surface() %>% 
  layout(scene = list(yaixs = list(autorange = "reversed"),
                      zaxis = list(range = c(1,150)),
                      image = list(
                        source = base64enc::dataURI(file ="file/nyc.jpg"),
                        x = ~min(x),
                        y = ~min(y),
                        sizex = 0.2, sizey = 0.1,
                        xanchor = "left",
...
TEST
```{r}
res = lm(fine_amount ~factor(borough), data = boro_amount)
anova(res)
res1 = aov(fine_amount ~factor(borough), data = boro_amount)
summary(res1)
