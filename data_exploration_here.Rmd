...
4. Risk map (risk is given by rank)
```{r}
nearby_data %>% 
  group_by(business_name) %>% 
  summarize(mean_fine = mean(fine_amount)) %>% 
  left_join(distinct(cafe), by = "business_name") %>% 
  drop_na(mean_fine) %>%
  mutate(percent_rank = rank(-mean_fine)/908,
         top_risk = ceiling(percent_rank*100)) %>% 
...
nearby_data %>% 
  group_by(business_name) %>% 
  summarize(mean_fine = mean(fine_amount)) %>% 
  ggplot(aes(x = mean_fine, fill = mean_fine))+
